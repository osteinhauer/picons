...
type Options struct {
	...
	PiconsRemoteFolderFallback string `short:"U" long:"picons-remote-folder-fallback" description:"Pfad der picons auf dem Server für fallback" default:"picons/uploader/LinuxLover2012/ASTRA19_LINUXLOVER2012_HD_PICONS_FLUID_300x130_32BIT_KOMPLETT"`
...
func (ref Ref) isSkipableName() bool {
	sidMatch, err := regexp.MatchString("^[0-9]+[aa-zA-Z]\\ SID\\ 0x([0-9]|[a-zA-Z])+$", ref.Servicename)
	if err != nil {
		log.Error("Falscher RegExp ", err)
		os.Exit(1)
	}
	dotMatch, err := regexp.MatchString("^\\(\\.+\\)$", ref.Servicename)
	if err != nil {
		log.Error("Falscher RegExp ", err)
		os.Exit(1)
	}
